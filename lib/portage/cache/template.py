...
class database:
    ...
    def __init__(
        self, location: str, label: str, auxdbkeys: frozenset, readonly: bool = False
    ) -> None:
    ...
    def __getitem__(self, cpv: Union[_pkg_str, str]) -> Dict[str, str]:
        """set a cpv to values
    ...
    def validate_entry(
        self, entry: Dict[str, str], ebuild_hash: hashed_path, eclass_db: cache
    ) -> bool:
        try:
            chf_types = self.chf_types
        except AttributeError:
...
def _md5_deserializer(md5: str) -> str:
    """
...
def reconstruct_eclasses(
    cpv: _pkg_str, eclass_string: str, chf_type: str = "mtime", paths: bool = True
) -> Dict[str, str]:
